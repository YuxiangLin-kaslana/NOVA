...
#   sbatch scripts/train_mlp_delta.sh

#SBATCH --job-name=sigla-mlp
...
#SBATCH --output=sigla-mlp-%j.out
#SBATCH --error=sigla-mlp-%j.err
...
"${PYTHON}" -m sigla_exp.train \
  ...
  --run_name detector_SMD_1-7_test_w50_s5 \
  --win_size 50 \
  --step 5 \
  --train_ratio 0.8 \
  --batch_size 128 \
...
"${PYTHON}" eval/eval_autoencoder.py \
  --run_dir /u/ylin30/sigLA/code/runs/detector_SMD_1-7_test_w50_s5 \
  --split test \
  --batch_size 128 \
  --num_workers 0
