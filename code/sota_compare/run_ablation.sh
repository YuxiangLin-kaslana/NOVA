...
#SBATCH --output=sigla-abl-%j.out
#SBATCH --error=sigla-abl-%j.err
set -euo pipefail
...
echo "##### synthetic #####"; "${PY}" sota_compare/exp_ablation_namer.py
for M in 1-1 2-5; do
  echo "##### machine-${M} #####"
  REAL_MACHINE="${M}" "${PY}" sota_compare/exp_ablation_namer.py
done
echo abl-done
